/*
 * @Author: Sliverkiss
 * @Date: 2023-05-19 10:05:27
 * @FilePath: https://github.com/Sliverkiss/helloworld/Study/jlc.js
 * @Description:
 * 嘉立创 每日签到
 * 捉u.lceda.cn域名下的Cookie中lceda_session=XXX,填写到mbym_cookie中,多帐号换行
 * 
 * 只用过loon，理论上支持qx、surge，请自行尝试
 * 
 * [Script]
 * cron "0 15 * * * MON" script-path=https://raw.githubsercontent.com/Sliverkiss/helloworld/main/Study/jlc.js, timeout=300, tag=嘉立创每日签到
 * 
 */

const $ = new Env("嘉立创");
const env_name = "jlc_cookie";
...
function checkin(user) {
    return new Promise((resolve) => {
        ...
        $.post(signinRequest, (error, response, data) => {
            ...
            try{
                if (result?.code == 0) {
                message += `🟢签到成功！本月已签到${result?.result?.mouthCount}天\n`;
            } else {
...
async function getCustomerPoints(user) {
    return new Promise((resolve) => {
        ...
        const signinRequest = {
            url: "https://u.lceda.cn/api/pointsExchange/totalPoints",
            headers: header,
        };
        $.post(signinRequest, (error, response, data) => {
            var result = JSON.parse(data);
            try{
                if (result?.code == 0) {
                message += `🟢用户状态查询成功！剩余${result?.result}积分\n`;
            } else {
                message += `🟡用户状态查询失败！${result?.message}\n`
            }
            }catch(e){
                $.msg($.name,`🔴${e}`,'');
            }finally{
