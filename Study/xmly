/*
 * @Author: Sliverkiss
 * @Date: 2023-05-18 09:23:18
 * @FilePath: https://github.com/Sliverkiss/helloworld/Study/stlxz.js
 * @Description:
 * 喜马拉雅 每日签到
 * 捉hybrid.ximalaya.com域名包下的Cookie中的1&_token=XXXXX;填写到xmly_cookie中，多账号换行
 * 
        ...
        $.post(signinRequest, (error, response, data) => {
            try {
                ...
                if (result?.data?.code==0) {
                    message += `账号[${user.index}] 🟢${result?.data?.msg}`;
                } else {
                    message += `账号[${user.index}]🟡${result?.data?.msg}`
                }
            } catch (error) {
...
async function getCookie() {
    if ($request && $request.method != "OPTIONS" && $request.url.match(/\/signIn\//)) {
