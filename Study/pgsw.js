...
const $ = new Env("苹果应用试玩");
//环境变量名字
const env_name = "pgsw_data";
...
function signin(user) {
    return new Promise((resolve) => {
        ...
        const signinRequest = {
            url: "https://shiwanxing.com/s5/reward.coin.claim.today",
            headers: header,
        };
        $.post(signinRequest, (error, response, data) => {
            ...
            if (result?.err_code == 0) {
                message += `\n帐号[${user.index}]签到成功！今日获得${result?.payload?.coin}金币`;
            } else {
                message += `\n帐号[${user.index}]签到失败！${result?.err_msg}`
            }
            resolve();
        });
    });
